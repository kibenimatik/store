...
module Services
  describe Services::Extensions::HasResponse do
    ...
    describe '#respond_with' do
      ...
      it 'calls Services::Response' do
        Services::Response.should_receive(:new).with(:foo, 'bar').and_return(mock.as_null_object)
        subject.respond_with('bar')
      end

      it 'should return Services::Response instance' do
        subject.respond_with('bar').should be_a_kind_of(Services::Response)
      end

      it 'should assign message, errors and additional assignments to response object' do
        response = mock
        Services::Response.should_receive(:new).and_return(response)
